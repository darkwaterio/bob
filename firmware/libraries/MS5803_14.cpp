...
static uint32_t     Z1 = 0;    // Store uncompensated pressure value
static uint32_t     Z2 = 0;    // Store uncompensated temperature value
...
boolean MS_5803::initializeMS_5803(boolean Verbose) {
    ...
    for (int i = 0; i < 8; i++ ){
    	...
    	Wire.beginTransmission((uint8_t) MS5803_I2C_ADDRESS);
    	Wire.write((uint8_t) (0xA0 + (i * 2)));
		//Wire.write(0x00);
...
void MS_5803::readSensor() {
	...
	// to read.
	if (_Resolution == 256){
		Z1 = MS_5803_ADC(CMD_ADC_D1 + CMD_ADC_256); // read raw pressure
		Z2 = MS_5803_ADC(CMD_ADC_D2 + CMD_ADC_256); // read raw temperature
	} else if (_Resolution == 512) {
		Z1 = MS_5803_ADC(CMD_ADC_D1 + CMD_ADC_512); // read raw pressure
		Z2 = MS_5803_ADC(CMD_ADC_D2 + CMD_ADC_512); // read raw temperature
	} else if (_Resolution == 1024) {
		Z1 = MS_5803_ADC(CMD_ADC_D1 + CMD_ADC_1024); // read raw pressure
		Z2 = MS_5803_ADC(CMD_ADC_D2 + CMD_ADC_1024); // read raw temperature
	} else if (_Resolution == 2048) {
		Z1 = MS_5803_ADC(CMD_ADC_D1 + CMD_ADC_2048); // read raw pressure
		Z2 = MS_5803_ADC(CMD_ADC_D2 + CMD_ADC_2048); // read raw temperature
	} else if (_Resolution == 4096) {
		Z1 = MS_5803_ADC(CMD_ADC_D1 + CMD_ADC_4096); // read raw pressure
		Z2 = MS_5803_ADC(CMD_ADC_D2 + CMD_ADC_4096); // read raw temperature
	}
    ...
    dT = (int32_t)Z2 - ( (int32_t)sensorCoeffs[5] * 256 );
...
//    TEMP = 2000 + ((int64_t)dT * sensorCoeffs[6]) / (int64_t)1<<23;
    ...
    // float (mbar).
	// For 14 bar sensor
	mbarInt = ((Z1 * Sensitivity) / 2097152 - Offset) / 32768;
